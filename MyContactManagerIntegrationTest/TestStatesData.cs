...
namespace MyContactManagerIntegrationTests
{
    public class TestStatesData
  {
    ...
    [Theory]
    [InlineData("Star Trek: The Next Generation", "STTNG", 0)]
    public async Task AddAndDeleteState(string expectedName, string expectedAbbreviation, int stateId)
    {
      using (var context = new MyContactManagerDbContext(_options))
      {
        _repository = new StatesRepository(context);
        ...
        var stateToAdd = new State() { Id = stateId, Name = expectedName, Abbreviation = expectedAbbreviation };
        ...
        updatedState.Id.ShouldBeGreaterThan(0);
        updatedState.Name.ShouldBe(expectedName, StringCompareShould.IgnoreCase);
        updatedState.Abbreviation.ShouldBe(expectedAbbreviation, StringCompareShould.IgnoreCase);
    ...
    public async Task UpdateState()
    {
      using (var context = new MyContactManagerDbContext(_options))
      {
        ...
        var stateToUpdate = await _repository.GetAsync(IOWA_ID); //expected IWOA -> purposeful typo
        ...
        var updatedState = await _repository.GetAsync(IOWA_ID); //expected IWOA -> purposeful typo
        updatedState.ShouldNotBeNull();
        ...
        var revertedState = await _repository.GetAsync(IOWA_ID); //expected IWOA -> purposeful typo
